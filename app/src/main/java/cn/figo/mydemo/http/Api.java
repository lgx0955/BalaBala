...
public interface Api {
    ...
    Observable<BangumiIndexBean> updateBangumi();
    //获取首页推荐页banner
    @GET("http://app.bilibili.com/x/banner?plat=4&build=411007&channel=bilih5")
    Observable<RecommendBannerBean> updateRecommendBanner();

    //获取首页番剧-新番完结分类
    @GET("http://bangumi.bilibili.com/api/app_index_page?_device=android")
    Observable<BanguminIndexHeaderBean> updateBangumiIndexHeader();
    //获取番局详情
    ...
    Observable<VideoDetailBean> getBangumiDetail();
    //获取番剧的瀑布流详情
    ...
    Observable<BangumiDetailBean> getBangumiDetailBySpId(@Query("sp_id") String sp_id,@Query("ts") String ts,@Query("type") String type);//sp
    //获取番剧lastupdate的详情
    ...
    Observable<BangumiDetailBean> getBangumiDetailBySeasonID(@Query("season_id") String season_id,@Query("ts") String ts,@Query("type") String type);//bangumi
    //获取番剧播放地址
    @GET("http://api.bilibili.com/playurl?page=1&platform=html5&quality=1&vtype=mp4&type=jsonp&_=1476304264934")
    Observable<String> getBangumiPlayUrl(@Query("aid") String aid);
    ...
    Observable<LiveIndexBean> updateLiveIndex();
    //获取直播地址
    @GET("http://live.bilibili.com/api/playurl?platform=h5")
    Observable<LiveVideoSourceBean> getLiveVideoSource(@Query("cid") String room_id);



    //获取视频弹幕
    @GET("http://comment.bilibili.com/{danmaku_id}.xml")
    Observable<ResponseBody> getDanmaku(@Path("danmaku_id") String danmaku_id);

    //获取视频地址
    @GET("http://www.bilibili.com/m/html5")
    Observable<AidBean> getVideoPath(@Query("aid") String aid, @Query("page") String page);
